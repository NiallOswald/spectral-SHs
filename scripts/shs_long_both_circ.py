"""Channel with longitudinal grooves on both walls and circular meniscus."""
...
alpha = (1 / (4 * np.sin(mu) ** 2) - (c - 1) ** 2 / 16) ** (-1 / 2)
beta = (
    1
    - l / (2 * np.tan(mu))
    + l * np.sqrt(1 / (4 * np.sin(mu) ** 2) - (c - 1) ** 2 / 16)
) ** (-1)

alphaf = np.tile(alpha, n + 1)
betaf = np.tile(beta, n + 1)
...
yy1 = (c.reshape(n + 1, 1) + 1) / 2 @ (beta**-1).reshape(1, n + 1)
...
L1 = (
    (16 / l**2) * D2x
    + (2 / l) * np.diag(alphaf * (xif - 1) * (etaf + 1)) @ Dxy
    + (1 / l) * np.diag(alphaf * (etaf + 1)) @ Dxy
    + (1 / (16 * l)) * np.diag(alphaf**3 * (xif - 1) ** 2 * (etaf + 1)) @ Dy
    + (1 / 16) * np.diag(alphaf**2 * (xif - 1) ** 2 * (etaf + 1) ** 2) @ D2y
    + (1 / 16) * np.diag(alphaf**2 * (xif - 1) ** 2 * (etaf + 1)) @ Dy
    + 4 * np.diag(betaf**2) @ D2y
)
...
L1[n :: n + 1, :] = (4 / l) * Dx[n :: n + 1, :] - np.tan(mu) * np.diag(
    c + 1
) @ Dy[n :: n + 1, :]
