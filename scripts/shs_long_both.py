...
L1[: n + 1, :] = Dy[: n + 1, :]  # Interface eta = 1
f[: n + 1] = 0

L1[(n + 1) * n :, :] = Dy[(n + 1) * n :, :]  # Symmetry eta = -1
f[: n + 1 : (n + 1) ** 2] = 0
...
L2[: n + 1, :] = np.eye(n + 1, (n + 1) ** 2)  # No-slip eta = 1
f[(n + 1) ** 2 : (n + 1) * (n + 2)] = 0

L2[(n + 1) * n :, :] = Dy[(n + 1) * n :, :]  # Symmetry eta = -1
f[(n + 1) * (2 * n + 1) * n :] = 0
...
uu = u.reshape((n + 1, 2 * (n + 1)))

# Create a finer grid for plotting
x_fine = np.linspace(-L / (2 * H), 0, 41)
